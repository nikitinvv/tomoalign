...
def myplot(u, psi, flow, binning):
    ...
    if not os.path.exists('/data/staff/tomograms/vviknik/tomoalign_vincent_data/ZP/flow/'):
        os.makedirs(
            '/data/staff/tomograms/vviknik/tomoalign_vincent_data/ZP/flow/')
    plt.savefig(
        '/data/staff/tomograms/vviknik/tomoalign_vincent_data/ZP/flow/'+str(k))
    plt.close()
...
if __name__ == "__main__":
    binning = 1
    prj = np.zeros([4000,1024,1224],dtype='float32')
    
    prj[:,:512] = np.load('/data/staff/tomograms/vviknik/tomoalign_vincent_data/ZP/prjbin1p1.npy').astype('float32')#[2:4]                               
    prj[:,512:] = np.load('/data/staff/tomograms/vviknik/tomoalign_vincent_data/ZP/prjbin1p2.npy').astype('float32')#[2:4]                               
    
    #prj[1024:]= np.load('/data/staff/tomograms/vviknik/tomoalign_vincent_data/ZP/prjbin0p1.npy').astype('float32')[1000:-1000]                               
    ...
    # np.save('/data/staff/tomograms/vviknik/tomoalign_vincent_data/ZP/prjbin1p2',prj)        
    # # #np.save('/data/staff/tomograms/vviknik/tomoalign_vincent_data/ZP/prjbin2p2',prj[:,256:])        
    # exit()
    dxchange.write_tiff_stack(prj,'/data/staff/tomograms/vviknik/tomoalign_vincent_data/ZP/databin1/r')
    #exit()
        ...
        with dc.SolverDeform(ntheta, nz, n, ptheta) as dslv:
            ...
            for k in range(niter):
                ...
                if(np.mod(k, 1) == 0):  # check Lagrangian
                    ...
                    dxchange.write_tiff_stack(
                        u.real,  '/data/staff/tomograms/vviknik/tomoalign_vincent_data/ZP/'+str(binning)+'_'+str(ntheta)+'/rect'+str(k)+'/r', overwrite=True)
                    # dxchange.write_tiff_stack(
                    #   psi.real, '/data/staff/tomograms/vviknik/tomoalign_vincent_data/ZP/'+str(binning)+'_'+str(ntheta)+'/psir'+str(k)+'/r',  overwrite=True)
