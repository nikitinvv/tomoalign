...
import dxchange
import tomoalign
from tomoalign.utils import tic,toc
if __name__ == "__main__":
    ...
    ptheta = 16
    # simulate data
    mmin, mmax = tomoalign.utils.find_min_max(data)
    with tomoalign.SolverDeform(pprot, nz, n, ptheta, ngpus) as dslv:
        # register flow
        tic()
        flow = None
        for k in range(4):
    ...
    dxchange.write_tiff(data1_unwrap,'data/deform/data1_unwrap',overwrite=True)
    dxchange.write_tiff(data0-data1_unwrap,'data/deform/difunwrap',overwrite=True)
