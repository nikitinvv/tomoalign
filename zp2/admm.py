...
matplotlib.use('Agg')
centers={
'/data/staff/tomograms/vviknik/tomoalign_vincent_data/zp/Kenan_ZP_9100eV_interlaced_30deg_2.5s_061': 522,
}
ngpus = 4
def myplot(u, psi, flow, binning,center):
    ...
    plt.subplot(3, 4, 12)
    plt.imshow(u[:, :, n//2], cmap='gray')
    if not os.path.exists(name+'/aflowfw_'+'_'+str(ntheta)):
        os.makedirs(
            name+'/aflowfw_'+'_'+str(ntheta))
    plt.savefig(
        name+'/aflowfw_'+'_'+str(ntheta)+'/'+str(center))
    plt.close()
...
if __name__ == "__main__":
    ...
    for il in range(1):
        binning = binnings[il]
        data = np.zeros([ndsets*nth,1024//pow(2,binning),1024//pow(2,binning)],dtype='float32')
       ...
       # exit()
        ...
        # pad data    
        ne = 1024//pow(2,binning)    
        #ne=n
        for ic in range(-35,-20):
            center = centers[sys.argv[3]]+(ne//2-n//2)*pow(2,binning)+ic
                ...
                with dc.SolverDeform(ntheta, nz, n, ptheta) as dslv:
                    ...
                    for k in range(niter[il]):
                        ...
                        flow = dslv.registration_flow_batch(
                            psi, data, mmin, mmax, flow.copy(), pars, 29) 
