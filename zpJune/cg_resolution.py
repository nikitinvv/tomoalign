...
if __name__ == "__main__":
    ...
    # data-=np.mean(data)
    # pad data    
    ne = (2048+1024+512)//pow(2,binning)    
    #ne=n
    print(data.shape)
    # data=prealign(data)
    ...
    with tc.SolverTomo(np.array(theta,order='C'), ntheta, nz, ne, pnz, center/pow(2, binning), ngpus) as tslv:
        ucg = tslv.cg_tomo_batch(pad(np.array(data,order='C'),ne,n),u,64)
        dxchange.write_tiff_stack(
            ucg[:,ne//2-n//2:ne//2+n//2,ne//2-n//2:ne//2+n//2],  name+'/cga'+'_'+str(ntheta)+'/rect'+str(k)+'/r', overwrite=True)
    ...
    #     dxchange.write_tiff_stack(
    #         ucg[:,ne//2-n//2:ne//2+n//2,ne//2-n//2:ne//2+n//2],  name+'/cga_resolution2_'+'_'+str(ntheta//2)+'/rect'+str(k)+'/r', overwrite=True)
