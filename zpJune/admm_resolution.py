...
def myplot(u, psi, flow, binning):
    ...
    plt.subplot(3, 4, 12)
    plt.imshow(u[:, :, n//2], cmap='gray')
    if not os.path.exists(name+'/flowfw_resolution'+str(st)+'_'+str(ntheta)):
        os.makedirs(
            name+'/flowfw_resolution'+str(st)+'_'+str(ntheta))
    plt.savefig(
        name+'/flowfw_resolution'+str(st)+'_'+str(ntheta)+'/'+str(k))
    plt.close()
...
if __name__ == "__main__":
    ...
    for il in range(3):
        ...
        data = np.array(data[st::2],order='C')
        theta = np.array(theta[st::2],order='C')        
        ...
        center = centers[name]+(ne//2-n//2)*pow(2,binning)        
                   ...
                   # print(np.linalg.norm(Tpsi-data))
                    ...
                    if(np.mod(k,4)==0):  # check Lagrangian
                        ...
                        dxchange.write_tiff_stack(
                            u[:,ne//2-n//2:ne//2+n//2,ne//2-n//2:ne//2+n//2],  name+'/fw_resolution'+str(st)+'_'+str(ntheta)+'/rect'+str(k)+'/r', overwrite=True)
                        dxchange.write_tiff_stack(
                        psi.real, name+'/fw_resolution'+str(st)+'_'+str(ntheta)+'/psi'+str(k)+'/r', overwrite=True)
                        if not os.path.exists(name+'/fw_resolution'+str(st)+'_'+str(ntheta)+'/flownpy'):
                          os.makedirs(name+'/fw_resolution'+str(st)+'_'+str(ntheta)+'/flownpy')
                        np.save(name+'/fw_resolution'+str(st)+'_'+str(ntheta)+'/flownpy/'+str(k),flow)
