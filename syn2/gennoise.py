...
def myplot(u, psi, flow, binning):
    ...
    plt.subplot(3, 4, 12)
    plt.imshow(u[:, :, n//2], cmap='gray')
    if not os.path.exists('/data/staff/tomograms/vviknik/tomoalign_vincent_data/syn/'+'/noiseflowfw_'+'_'+str(ntheta)+str(alpha)):
        os.makedirs(
            '/data/staff/tomograms/vviknik/tomoalign_vincent_data/syn/'+'/noiseflowfw_'+'_'+str(ntheta)+str(alpha))
    plt.savefig(
        '/data/staff/tomograms/vviknik/tomoalign_vincent_data/syn/'+'/noiseflowfw_'+'_'+str(ntheta)+str(alpha)+'/'+str(k))
    plt.close()
...
with tc.SolverTomo(theta, ntheta, nz, ne, 256, n/2+(ne-n)/2, 1) as tslv:
    with dc.SolverDeform(ntheta, nz, n, 16) as dslv:
        ...
        #exit()
        # data[data<0]=0
        # data=np.random.poisson(data/4).astype('float32')*4
        # dxchange.write_tiff(data,'/data/staff/tomograms/vviknik/tomoalign_vincent_data/syn/datadnoise.tiff',overwrite=True)
        # #exit()
        ...
        for k in range(256):
            ...
            if(np.mod(k,4)==0):  # check Lagrangian
                ...
                print("%d %d %.2e %.2f %.2f %.4e %.4e %.4e %.4e " % (k, pars[2], np.linalg.norm(
                    flow), rho, rho1, *lagr))
                sys.stdout.flush()           
                dxchange.write_tiff_stack(
                    u[:,ne//2-n//2:ne//2+n//2,ne//2-n//2:ne//2+n//2],  '/data/staff/tomograms/vviknik/tomoalign_vincent_data/syn/'+'/noisefw_'+'_'+str(ntheta)+str(alpha)+'/rect'+str(k)+'/r', overwrite=True)
                dxchange.write_tiff_stack(
                psi.real, '/data/staff/tomograms/vviknik/tomoalign_vincent_data/syn/'+'/noisefw_'+'_'+str(ntheta)+str(alpha)+'/psi'+str(k)+'/r', overwrite=True)
                np.save('/data/staff/tomograms/vviknik/tomoalign_vincent_data/syn/'+'/noisefw_'+'_'+str(ntheta)+str(alpha)+'/flownpy/'+str(k),flow)
